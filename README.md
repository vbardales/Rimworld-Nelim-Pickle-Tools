...
| [`SoundCapture/`](SoundCapture/README.md) | **Optional.** Record what the game plays between two steps and measure it (or film it as a video with its sound), or assert that the game holds a playing sound; not in the bundle. Tests that record run in the WSL like the others and play on the owner's speakers for a few seconds; they start with the volume step, the WSL profile being muted | `nelim.pickletools.soundcapture` |
| [`NewColony/`](NewColony/README.md) | **Optional, costly.** Start a new colony from the main menu with the scenario, storyteller, difficulty, map size and seed fixed. Not reproducible, minutes of generation, watchdog to raise: played once, in an initial or a final pass; not in the bundle | `nelim.pickletools.newcolony` |
| [`LoadAudit/`](LoadAudit/README.md) | **Optional.** Assert that the load of a mod is clean: errors, warnings and unresolved definitions that belong to it in the game log, repeated messages, keyed translations missing from the active language; not in the bundle | `nelim.pickletools.loadaudit` |
