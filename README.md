...
| [`SoundCapture/`](SoundCapture/README.md) | **Optional.** Record what the game plays between two steps and measure it, or assert that the game holds a playing sound; not in the bundle. Tests that record run in the WSL like the others and play on the owner's speakers for a few seconds; they start with the volume step, the WSL profile being muted | `nelim.pickletools.soundcapture` |
