...
| `AnimaSongSteps.cs` | **Sit or not sit: is a group of pawns spread on distinct cells in a ring around a thing**: `{n} listeners sit on {m} different cells around the tree`. Tied to the mod's job and ring; kept only for the way it waits for the pawns to arrive before it counts. |
