...
Steps: `Tests/Pickle/Source/InstrumentSteps.cs`, one class, 6 steps, built by
...
| `the recipe {string} is {word}` | `RecipeDef.AvailableNow` (`available` or `unavailable`), the question a bench's bill menu asks: false while `researchPrerequisite` is not finished. Combined with Pickle's `research {string} is finished` it shows a recipe appearing with its research. | **Yes**, for any mod whose recipes wait for a research. Only the prefix is ours. |
