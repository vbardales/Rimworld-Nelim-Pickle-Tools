...
| `ScreenshotSteps.cs` | `SkillIcons hides the interface around the windows on screen`, `SkillIcons brings the interface back` | Turns on RimWorld's own screenshot mode so that only windows draw, while keeping `drawInScreenshotMode` enabled on non-Pickle windows and disabling it on Pickle windows. Restores from an explicit step **and** from `[AfterScenario]`, so a scenario that dies between the two does not leave every later scenario photographing a screen with no interface. Different mechanism from `ClearScreen`, which keeps other mods' windows away; this hides the HUD. Caveat: a publication capture taken with it in a headless run showed the settings window correctly; see the next row for the one that did not. |
...
### Shared version exists; local copy remains

Source inventory, 2026-09-22: `SkillIcons/Tests/Pickle/Source/TextureOwnerSteps.cs` still exists.
The shared version is [TextureOwner](../TextureOwner/README.md), whose adoption instructions describe the
phrase and pass-map changes. Do not report the suite migration as complete merely because the shared DLL exists.
