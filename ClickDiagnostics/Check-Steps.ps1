<#
.SYNOPSIS
  Compile the step patterns of the click diagnostics steps with Pickle's own expression engine, and check
...
$mine = @(Read-Patterns (Join-Path $here 'Source') 'shared:ClickDiagnostics')
...
foreach ($file in $featureFiles) {
    foreach ($raw in [IO.File]::ReadAllLines($file.FullName)) {
        if ($raw.Trim() -notmatch '^(Given|When|Then|And|But)\s+(.+)$') { continue }
        $step = $Matches[2].Trim()
        if ($step -notmatch "^Nelim's Pickle Tools: (the button keyed|I click the button keyed)") { continue }
