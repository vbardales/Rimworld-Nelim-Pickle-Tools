<#
.SYNOPSIS
  Compile the step patterns of the new colony steps with Pickle's own expression engine, and check
...
.EXAMPLE
  powershell.exe -ExecutionPolicy Bypass -File PickleTools/NewColony/Check-Steps.ps1
#>
param(
...
$mine = @(Read-Patterns (Join-Path $here 'Source') 'shared:NewColony')
...
# The features that use this vocabulary must resolve completely: any line of this vocabulary no expression matches.
foreach ($file in $featureFiles) {
    foreach ($raw in [IO.File]::ReadAllLines($file.FullName)) {
        if ($raw.Trim() -notmatch '^(Given|When|Then|And|But)\s+(.+)$') { continue }
        $step = $Matches[2].Trim()
        if ($step -notmatch "^Nelim's Pickle Tools: (a new colony is started|the new colony's)") { continue }
...
if ($unresolved.Count -gt 0) {
    Write-Host ''
    Write-Host "$($unresolved.Count) step line(s) of this vocabulary match no expression at all (shared, other suites, Pickle):" -ForegroundColor Red
...
Write-Host 'ALL PATTERNS COMPILE, NONE DECLARED TWICE, NONE AMBIGUOUS, EVERY LINE OF THIS VOCABULARY RESOLVES' -ForegroundColor Green
exit 0
