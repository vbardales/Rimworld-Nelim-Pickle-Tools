<#
.SYNOPSIS
  Compile the step patterns of the interface scale step with Pickle's own expression engine, and check
...
$mine = @(Read-Patterns (Join-Path $here 'Source') 'shared:InterfaceScale')
...
# The features that use this vocabulary must resolve completely: any interface-scale line no expression matches.
foreach ($file in $featureFiles | Where-Object { $_.Name -like '*interface-scale*' }) {
    foreach ($raw in [IO.File]::ReadAllLines($file.FullName)) {
        if ($raw.Trim() -notmatch '^(Given|When|Then|And|But)\s+(.+)$') { continue }
        $step = $Matches[2].Trim()
...
if ($unresolved.Count -gt 0) {
    Write-Host ''
    Write-Host "$($unresolved.Count) step line(s) of the interface-scale features match no expression at all (shared, other suites, Pickle):" -ForegroundColor Red
...
Write-Host 'ALL PATTERNS COMPILE, NONE DECLARED TWICE, NONE AMBIGUOUS, EVERY INTERFACE-SCALE LINE RESOLVES' -ForegroundColor Green
exit 0
