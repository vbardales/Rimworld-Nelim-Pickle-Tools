<#
.SYNOPSIS
  Compile the step patterns of the shared film-ticks steps with Pickle's own expression engine, and check
...
$mine = @(Read-Patterns (Join-Path $here 'Source') 'shared:FilmTicks')
...
# Every step line, in any suite, that carries this tool's VERBS must resolve completely: a mistyped text would otherwise be an undefined step in the middle of a run. The prefix alone is shared with the sibling tools, whose patterns this script does not read, so it cannot be the filter.
foreach ($file in $featureFiles) {
    foreach ($raw in [IO.File]::ReadAllLines($file.FullName)) {
        if ($raw.Trim() -notmatch '^(Given|When|Then|And|But)\s+(.+)$') { continue }
        $step = $Matches[2].Trim()
        if (-not ($step.StartsWith("Nelim's Pickle Tools: I film ") -or $step -eq "Nelim's Pickle Tools: I stop filming")) { continue }
...
if ($unresolved.Count -gt 0) {
    Write-Host ''
    Write-Host "$($unresolved.Count) film-ticks step line(s) match no expression at all (shared, other suites, Pickle):" -ForegroundColor Red
...
Write-Host 'ALL PATTERNS COMPILE, NONE DECLARED TWICE, NONE AMBIGUOUS, EVERY FILM-TICKS LINE RESOLVES' -ForegroundColor Green
exit 0
