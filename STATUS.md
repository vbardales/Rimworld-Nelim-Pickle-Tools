...
updated: 2026-09-25
remaining:
  - "SoundCapture, 2026-09-25 (owner): tests that use it are run ALONE and on WINDOWS, the mod list changed for the run and restored after. This is an exception to AUDIT.md's rule that no session launches the Windows game, given for these tests only; nothing was launched, none is to be until she asks, and no Windows launcher exists (Run-PickleWsl.ps1 is WSL only, Run-Pickle.ps1 -Launch refuses). Still to do: the recording itself on Windows (the tool records through PulseAudio only, a dshow or loopback input is not written). The WSL scenario measured -91 dB once, cause unknown."
