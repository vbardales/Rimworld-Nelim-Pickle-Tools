...
stage: done
stage_meaning: "Final publication audit on 2026-09-22 verified the standalone repository identity, public visibility exception, original MIT licensing and distributed attribution; ModIcon and Preview artifacts; settings and localization as justified not_applicable; English publication documentation; and clean offline builds/checks for all thirteen distributed modules. On 2026-09-25 the preTest -> done criteria of AUDIT.md were checked: functional scenarios written as Gherkin features with preconditions, actions and expected results (their scope and what stays manual are in TESTING.md, "Functional scenarios and their scope"); automated offline tests written, run on the current tree and green (see automated_tests); Pickle scenarios written, none executed for this step, as AUDIT.md requires; no XML tests (the repository has no Defs, see TESTING.md). done means ready for the final in-game functional validation, not tested: the aggregate in-game validation, the manual checks and the SoundCapture decision are the criteria of done -> tested and are listed in remaining. 2026-09-25 (owner): 1.0.0 is published, and the repository works on the next release, so the stage returned to preTest while HoverSteps entered the bundle (fourteen modules) with nothing checked or written for it; it is back to done the same day, the preTest -> done criteria being met again on the fourteen-module tree (see automated_tests and remaining). The aggregate matrix on the fourteen-module payload belongs to done -> tested."
licence: original
...
automated_tests: "2026-09-25, on the fourteen-module tree (dfd6978 plus the HoverSteps scenario), offline: all fifteen Check-*.ps1 scripts pass, HoverSteps/Check-Steps.ps1 included (6 patterns, none ambiguous against 1207 others), and Release/Prepare-Release.ps1 -Check passes (14 DLLs identical to the tracked ones). Earlier the same day: all fourteen Check-*.ps1 scripts pass (two were red for a reason of the checker, not of a tool, and were fixed: InterfaceScale/Check-Steps.ps1 did not read the sibling tools' steps, which its scenario now uses, and RimmsqolSteps/Check-Steps.ps1 could not read attributes written as Prefix + a literal, ScreenshotStudio's); FilmTicks unit tests 7/7 (the test project now targets net8.0, so that the SDK on the PATH, 8.0.424, runs it; a .NET 10 SDK is also installed at C:\Users\nelim\.dotnet10); SoundCapture unit tests 13/13; QuietNewFactions Run-Behavior.ps1 6 checks passed. Earlier, 2026-09-22: the thirteen scripts passed after the full rebuild. ScreenshotMode attributes were changed from constant concatenation to literal expressions so the repository-wide vocabulary checker can see them. ColonistRace and InspectTabs still have no individual Check-Steps script. These are offline results, not aggregate runtime validation."
...
updated: 2026-09-25
remaining:
  - "next release, decisions of 2026-09-25 relayed by the CI/CD session as confirmed by the owner in its conversation: (1) the version is 1.1.0; (2) the rollback target for this first CI publication is Steam's own 'rétablir cette version' in the item's change history, reverting to 1.0.0, chosen by her by hand if the published 1.1.0 turns out wrong (v1.0.0, 2dc9845, cannot be a CI rollback target: no .github, no DLL in its Mod/). Unchanged: the gates (tested, prepublished, gallery, her manual validations, a green replay of any red) come before any publish, the dry-run of the exact SHA is mandatory, only she approves. A dry-run of dfd6978 as 1.1.0 with update_description passed (run 36140757130, nothing sent to Steam). The final SHA is not yet: the description's tested sentence must be true for what was played (fourteen modules), see PUBLICATION.md."
  - "SoundCapture, 2026-09-25 (owner): the rule that its tests run alone and on Windows was given and WITHDRAWN the same day, the WSL recording fine: with the volume step at 80 percent the sink scenario passed in WSL (ticket 0ce6, peak -16.2 dB, 1 of 1) and the owner heard the menu music, which also plays on her speakers while it is recorded; the game-side check passed too (ticket f3c5). The WSL staging writes volumeMaster 0, which was the earlier -91 dB. No session launches the Windows game; SoundCapture/Run-Windows.ps1 is hers, for listening. Still open: the recording itself is a level, not a judgement that the sound is the right one (the sound.wav of the run is kept for her to listen to)."
  - "done, 2026-09-25: HoverSteps joined the bundle on the owner's word. Written here: Tests/Pickle/.../pickletools-hoversteps.feature (one scenario on the options dialog, keyed click then hover by key), its map wsl-deps.hoversteps.map, and TESTING.md's scope row; offline check green. Played 2026-09-25 in English: 1 of 1 (the tooltip of the autosave interval, hovered by its key and drawn, capture read; a first play on the volume slider's key had failed because the slider registers no tooltip); not played in French. The body type step (ColonistRace) was played too: 3 of 3 in English, adult words and Child, after a first Child play failed on an adult shirt without a child texture (fixed: the step takes off apparel the stage may not wear); Baby and French not played. The description in About.xml and PUBLICATION.md was reworded on 2026-09-25 (owner: B) to say the matrix ran on the thirteen tools and that the steps added since have scenarios of their own. Also open for done -> tested: the aggregate matrix replayed on the fourteen-module payload; and the PUBLICATION.md description says the bundle was tested across a restart, which must be true at the moment a publish replaces the page (reword it to what was played if the matrix is not replayed with HoverSteps)."
  - "dry-run, 2026-09-25: GREEN for SHA f236105ec36798e80dd6bfa77e79e918a776cf7d, version 1.1.0, update_description=true (other options off): GitHub Actions run 36175171078 (https://github.com/vbardales/Rimworld-Nelim-Pickle-Tools/actions/runs/36175171078), launched by the CI/CD session. Payload check passed (14 DLL, no Source, no .build); the change note is the `### 1.1.0` block; the description is 3295 bytes (sha256 6d2dc4484c2377fa1d1afcc402a3bf0d0d8243b31e8024d5b5490000b9072982), and its diff against the live page is the three lines of PUBLICATION.md. THE PUBLISH MUST TARGET THAT SHA, f236105e..., NOT THE HEAD OF main: this note and every later commit change the SHA, and the dry-run is valid for that exact commit only. Still open before a publish (fail fast): the Workshop gallery (no folder declared, no capture exists), the owner's manual validations (the @review captures in English and French, the real click on RIMMSQOL's checkbox), the rollback chosen by the owner by hand in Steam's history, and her approval of steam-production. No session approves a publish."
  - "description source, decided 2026-09-25 by the CI/CD session (Rimworld-Release-Admin f196148, docs/OPERATIONS.md; Rimworld-protocols PUBLISHING.md 16f3c59): one Markdown description in a ```markdown block under ## Steam description of PUBLICATION.md, from which the CI generates the BBCode and the About.xml <description>. **Do not migrate before the 1.1.0 publication** (the dry-run f236105 is green with the BBCode block, and a migration changes the SHA); adopt it at the next release, or when the owner asks: ask the CI/CD session to regenerate the workflow (--description-markdown --about-from-description), then `node .github/scripts/sync-about-description.mjs --write` and read the About.xml diff. The CI now also refuses a change note whose first line does not carry the version; PUBLICATION.md's `### 1.1.0` block already starts with [h3]1.1.0[/h3]."
  - "released, 2026-09-25 (owner): version 1.0.0 was released on 2026-09-22 (Steam item 3806142401, public per Steam's API; GitHub release v1.0.0 on 2dc9845). The reading session deleted the GitHub release and tag by mistake on 2026-09-25 and recreated them the same day on the same commit with the original text; its three attached files are lost (see docs/PROTOCOLS-READ.md). The stage field above is not changed by this note: whether it should read published is the owner's call. Next release: no publish workflow exists yet (put to the CI/CD session), the description is corrected in About.xml and PUBLICATION.md but not on the live page, and the payload is not committed."
  ...
  - "resolved, 2026-09-22 (corrected 2026-09-25: the item is public, not private, per Steam's public API): the Workshop item is 3806142401. Mod/About/PublishedFileId.txt records it and release packaging preserves it; this prevents a later upload from creating a duplicate item."
