...
remaining:
  ...
  - "SoundCapture, 2026-09-25 (owner): tests that use it are run ALONE and on WINDOWS, the mod list changed for the run and restored after. This is an exception to AUDIT.md's rule that no session launches the Windows game, given for these tests only; nothing was launched, none is to be until she asks. The exception is now written in AUDIT.md (protocols repository) and the dedicated launcher SoundCapture/Run-Windows.ps1 exists, checked as a dry run only (it changes the mod list in a profile of its own, -savedatafolder, and removes the Mods/ junctions it adds). The WSL staging writes volumeMaster 0, which was the -91 dB: with the volume step at 80 percent the sink scenario PASSED in WSL on 2026-09-25 (ticket 0ce6, peak -16.2 dB, 1 of 1), and the owner heard the menu music; the game-side check passed too (ticket f3c5, 1 of 1). So the recording works in WSL and whether the rule 'alone, on Windows' still holds is hers to say; Windows ffmpeg has no loopback device. Still to do: the recording itself on Windows (the tool records through PulseAudio only, a dshow or loopback input is not written). The WSL scenario measured -91 dB once, cause unknown."
  - "done, 2026-09-25: HoverSteps joined the bundle on the owner's word. Written here: Tests/Pickle/.../pickletools-hoversteps.feature (one scenario on the options dialog, keyed click then hover by key), its map wsl-deps.hoversteps.map, and TESTING.md's scope row; offline check green. First play queued (ticket 9eb1); the scenario assumes the volume slider registers a tooltip region under the key MasterVolumeTooltip and is not yet played. Also open for done -> tested: the aggregate matrix replayed on the fourteen-module payload; and the PUBLICATION.md description says the bundle was tested across a restart, which must be true at the moment a publish replaces the page (reword it to what was played if the matrix is not replayed with HoverSteps)."
