<#
.SYNOPSIS
  Compile the VEF faction step patterns with Pickle's own expression engine, and check
...
$mine = @(Read-Patterns (Join-Path $here 'Source') 'shared:VefFactionSteps')
...
# The features that use this vocabulary must resolve completely.
$consumerFiles = @($featureFiles | Where-Object {
    [IO.File]::ReadAllText($_.FullName).Contains("Nelim's Pickle Tools:") -and
    [IO.File]::ReadAllText($_.FullName) -match '(?m)^\s*(Given|When|Then|And|But)\s+Nelim''s Pickle Tools: (?:the load has settled|a faction the world lacks|the chosen faction|VEF runs|no new faction window|a new faction window|the game log says)'
})
...
if ($unresolved.Count -gt 0) {
    Write-Host ''
    Write-Host "$($unresolved.Count) step line(s) of the VEF-faction consumer features match no expression at all (shared, other suites, Pickle):" -ForegroundColor Red
...
Write-Host 'ALL PATTERNS COMPILE, NONE DECLARED TWICE, NONE AMBIGUOUS, EVERY VEF-FACTION CONSUMER LINE RESOLVES' -ForegroundColor Green
exit 0
