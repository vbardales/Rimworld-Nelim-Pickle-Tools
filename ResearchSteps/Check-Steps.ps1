<#
.SYNOPSIS
  Compile the step patterns of the shared research window steps with Pickle's own expression engine, and check
...
.EXAMPLE
  powershell.exe -ExecutionPolicy Bypass -File PickleTools/ResearchSteps/Check-Steps.ps1
#>
param(
...
$mine = @(Read-Patterns (Join-Path $here 'Source') 'shared:ResearchSteps')
...
# The features that use this vocabulary must resolve completely: any research line no expression matches.
foreach ($file in $featureFiles | Where-Object { $_.Name -like '*pickletools-research*' }) {
    foreach ($raw in [IO.File]::ReadAllLines($file.FullName)) {
        if ($raw.Trim() -notmatch '^(Given|When|Then|And|But)\s+(.+)$') { continue }
        $step = $Matches[2].Trim()
...
if ($unresolved.Count -gt 0) {
    Write-Host ''
    Write-Host "$($unresolved.Count) step line(s) of the pickletools-research features match no expression at all (shared, other suites, Pickle):" -ForegroundColor Red
...
Write-Host 'ALL PATTERNS COMPILE, NONE DECLARED TWICE, NONE AMBIGUOUS, EVERY RESEARCH LINE RESOLVES' -ForegroundColor Green
exit 0
