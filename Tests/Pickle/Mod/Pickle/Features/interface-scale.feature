...
# as the one proposed to Pickle itself in pull request 23, with the tool's prefix on the step, except that the button is clicked by
# the key its label comes from (KeyedClick): a click on the English label "New colony" cannot find its button in a French game.
