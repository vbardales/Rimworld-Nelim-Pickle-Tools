<#
.SYNOPSIS
  Compile the step patterns of the expansion steps with Pickle's own expression engine, and check
...
$mine = @(Read-Patterns (Join-Path $here 'Source') 'shared:ExpansionSteps')
...
# The features that use this vocabulary must resolve completely: any expansion line no expression matches.
foreach ($file in $featureFiles | Where-Object { $_.Name -like '*pickletools-expansions*' }) {
    foreach ($raw in [IO.File]::ReadAllLines($file.FullName)) {
        if ($raw.Trim() -notmatch '^(Given|When|Then|And|But)\s+(.+)$') { continue }
        $step = $Matches[2].Trim()
...
if ($unresolved.Count -gt 0) {
    Write-Host ''
    Write-Host "$($unresolved.Count) step line(s) of the pickletools-expansions features match no expression at all (shared, other suites, Pickle):" -ForegroundColor Red
...
Write-Host 'ALL PATTERNS COMPILE, NONE DECLARED TWICE, NONE AMBIGUOUS, EVERY EXPANSION LINE RESOLVES' -ForegroundColor Green
exit 0
