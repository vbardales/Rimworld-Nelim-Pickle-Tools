<#
.SYNOPSIS
  Compile the step patterns of the load audit steps with Pickle's own expression engine, and check
...
.EXAMPLE
  powershell.exe -ExecutionPolicy Bypass -File PickleTools/LoadAudit/Check-Steps.ps1
#>
param(
...
$mine = @(Read-Patterns (Join-Path $here 'Source') 'shared:LoadAudit')
...
foreach ($file in $featureFiles) {
    foreach ($raw in [IO.File]::ReadAllLines($file.FullName)) {
        if ($raw.Trim() -notmatch '^(Given|When|Then|And|But)\s+(.+)$') { continue }
        $step = $Matches[2].Trim()
        if ($step -notmatch "^Nelim's Pickle Tools: the load of the mod") { continue }
